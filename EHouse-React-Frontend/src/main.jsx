...
function App(){
  ...
  return <div className="app">
    <header className="nav">
      <button className="brand" onClick={()=>go("home")}><span className="brandMark">EH</span><span>E-House</span></button>
      <nav>
        {!auth && <>
  ...
  {auth.image ? <img src={auth.image} alt="" /> : <span>{initials(auth.name)}</span>}{auth.name}
        ...
        : <button className="outlineBtn" onClick={()=>{setAuthMode("login");go("auth")}}>Sign in</button>}
        {!auth && <button className="primaryBtn" onClick={()=>{setAuthMode("register");setAuthRole("customer");go("auth")}}>Join E-House</button>}
        {auth && <button className="iconBtn" title="Sign out" onClick={logout}>↗</button>}
      </div>
    </header>
    ...
    {page==="dashboard" && <CustomerDashboard auth={auth} houses={houses} go={go} setNotice={setNotice} setSelected={setSelected}/>}
    {page==="seller" && <SellerDesk auth={auth} go={go} setNotice={setNotice}/>}
    {page==="admin" && <AdminDesk setNotice={setNotice} go={go}/>}
    <footer className={auth ? "dashboardFooter" : ""}><div><span className="brandMark small">EH</span> <b>MAHA E-HOUSING</b> — digital property selling.</div><span>Built for simple, transparent home buying.</span></footer>
  </div>
}
...
function Home({houses,loading,query,setQuery,openHouse,go}){
 return <main>
  ...
  <section className="section">
   <div className="sectionHead"><div><span className="eyebrow">THE COLLECTION</span><h2>Homes with a story.</h2></div><button className="textBtn" onClick={()=>setQuery("")}>Clear search ↗</button></div>
...
function Details({house,auth,go,setNotice}){
 ...
 return <main className="pagePad"><button className="backBtn" onClick={()=>go("home")}>← Back to collection</button>
   ...
   try{
     if(mode==="register"){
       ...
       const data=await api("/auth/customer/register",{method:"POST",body:JSON.stringify({
         name:form.name,email:form.email,phone:form.phone,address:form.address,nida:form.nida,password:form.password
       })});
...
function DashboardSidebar({role, name, image, tab, setTab, items, go, actionLabel, onAction}){
 return <aside className="luxSidebar">
   ...
   <div className="sideBottom">
     ...
     <button className="sideHome" onClick={()=>go("home")}>← <span>Back to E-House</span></button>
   </div>
 </aside>
}

function CustomerDashboard({auth,houses,go,setNotice,setSelected}){
...
function SellerDesk({auth,go,setNotice}){
...
function Modal({title,onClose,children}){return <div className="modalBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div className="modalBox"><div className="modalHead"><div><span className="eyebrow">MAHA E-HOUSING</span><h2>{title}</h2></div><button className="modalClose" onClick={onClose}>×</button></div>{children}</div></div>}

function AdminPeopleCollection({title,type,data,onEdit,onDelete,onSelect,onAdd}){
 const [query,setQuery]=useState("");
 const filtered=data.filter(row=>`${row.name||""} ${row.email||""} ${row.nida||""} ${row[type==="sellers"?"sellerId":"customerId"]||""}`.toLowerCase().includes(query.toLowerCase()));
 return <div className="peopleCollection"><div className="collectionHead"><div><span className="eyebrow">DIRECTORY</span><h2>{title}</h2><p className="muted">{data.length} registered {type}.</p></div><div className="collectionActions">{onAdd&&<button className="primaryBtn" onClick={onAdd}>+ Add seller</button>}<input className="collectionSearch" value={query} onChange={e=>setQuery(e.target.value)} placeholder={`Search ${type}...`}/></div></div>{filtered.length?<div className="peopleGrid">{filtered.map((row,i)=>{const id=type==="sellers"?row.sellerId:row.customerId;return <article className={`personCard selectable ${onSelect?"clickable":""}`} key={id||i} onClick={()=>onSelect&&onSelect(row)}>{row.image?<img className="personPhoto" src={row.image} alt=""/>:<div className="personPhoto placeholder">{initials(row.name||"User")}</div>}<div className="personInfo"><div className="personTop"><span className="personRole">{type==="sellers"?"SELLER":"CUSTOMER"}</span><b>#{id??"—"}</b></div><h3>{row.name||"Unnamed user"}</h3><div className="personDetails"><span><strong>Email</strong>{row.email||"—"}</span><span><strong>NIDA</strong>{row.nida||"—"}</span></div>{onEdit&&<div className="personActions"><button className="smallBtn" onClick={e=>{e.stopPropagation();onEdit(row)}}>Edit</button><button className="smallBtn dangerBtn" onClick={e=>{e.stopPropagation();onDelete(row)}}>Delete</button></div>}</div></article>})}</div>:<div className="empty"><h3>No {type} found</h3><p>Try another search.</p></div>}</div>
}

function AdminDesk({setNotice,go}){
 ...
 return <main className="dashboardShell"><DashboardSidebar role="ADMIN" name="Administrator" tab={tab} setTab={setTab} go={go} actionLabel={tab==="sellers"?"Add seller":tab==="sales"?"Print seller report":null} onAction={tab==="sellers"?openAddSeller:printSellerSales} items={[{id:"overview",label:"Overview",icon:"◈"},{id:"sellers",label:"Sellers",icon:"♙"},{id:"customers",label:"Customers",icon:"♧"},{id:"houses",label:"All houses",icon:"⌂"},{id:"bookings",label:"Bookings",icon:"◷"},{id:"payments",label:"Payments",icon:"◇"},{id:"sales",label:"Sales by seller",icon:"✓"}]}/><section className="dashboardMain pagePad"><div className="dashTop"><div><span className="eyebrow">ADMIN PORTAL</span><h1>MAHA E-HOUSING<br/>{title}.</h1><p>{tab==="sellers"?"Manage seller accounts and open an individual seller record.":tab==="customers"?"Customer directory with identity details.":"Use the sidebar to open one management area at a time."}</p></div>{tab==="sellers"&&<button className="primaryBtn" onClick={openAddSeller}>+ Add seller</button>}{tab==="sales"&&<button className="primaryBtn" onClick={printSellerSales}>Print report →</button>}</div>
